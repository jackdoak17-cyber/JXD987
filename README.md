...
- The scheduled `Sync Odds` matrix reads `config/odds_api_leagues.json` but excludes the cup competitions listed in `config/odds_api_sync_excluded_leagues.json`; this prevents paid odds calls for cup data with no provider coverage.
- The core `scripts/sync_odds.py` guard and VPS defaults enforce the same policy; use `--include-excluded` only for an intentional, explicit cup override.
