...
class FixtureSettlementContractTests(unittest.TestCase):
    ...
    def test_odds_p3_lane_is_bounded_and_keeps_run_p3_identity(self) -> None:
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        source = wrapper.read_text(encoding="utf-8")
        self.assertIn("run_with_dedicated_lock_and_timeout", source)
        ...
        self.assertIn('ODDS_SYNC_P3_MAX_DURATION_SECONDS="${ODDS_P3_APPLY_MAX_RUNTIME_SECONDS:-300}"', source)
        self.assertIn("--priority settled-history", source)
        self.assertIn("--priority p3", source)
        self.assertIn('--stage-out "${HISTORY_STAGE}"', source)
        self.assertIn('--stage-out "${P3_STAGE}"', source)
        self.assertIn("--apply-stage", source)
        self.assertIn("validate_odds_api_market_catalog.py", source)
        self.assertIn("validate_moneyline_coverage.py", source)
        self.assertEqual(source.count('--priority p3'), 1)
        self.assertNotIn("MONEYLINE_REPAIR_ATTEMPTS", source)
        self.assertNotIn("--refresh-history", source)
    ...
    def test_stats_reconciliation_supervisor_is_bounded(self) -> None:
        ...
        self.assertIn("run_with_global_lock_and_retry", source)
        self.assertIn('ODDS_SYNC_LOCK_RETRY_ATTEMPTS="${STATS_RECONCILE_LOCK_RETRY_ATTEMPTS:-60}"', source)
        self.assertIn('ODDS_SYNC_LOCK_RETRY_DELAY_SECONDS="${STATS_RECONCILE_LOCK_RETRY_DELAY_SECONDS:-15}"', source)
    ...
    def test_experimental_model_lane_uses_a_separate_bounded_lock(self) -> None:
        ...
        self.assertIn('job_label="${4:-experimental model}"', common)
        self.assertIn('${job_label} process exceeded', common)
