alter table public.fixtures
  add column if not exists home_ht_score integer;

alter table public.fixtures
  add column if not exists away_ht_score integer;

create or replace function public.team_over_goals_first_half_rate_last_n(
  p_league_id integer,
...
as $$
  with appearances as (
    select
      ...
      (f.home_ht_score + f.away_ht_score)::numeric as total_goals,
      true as is_home
    from fixtures f
    where f.league_id = p_league_id
      and (p_season_id is null or f.season_id = p_season_id)
      and f.home_ht_score is not null
      and f.away_ht_score is not null
    union all
    select
      f.id as fixture_id,
      f.starting_at,
...
create or replace function public.team_over_goals_first_half_streak_last_n(
  p_league_id integer,
  p_goals_threshold numeric,
  p_n integer,
  p_required integer,
...
as $$
  with rate as (
    select *
    from public.team_over_goals_first_half_rate_last_n(
      p_league_id,
      p_goals_threshold,
      p_n,
      p_season_id,
...
create or replace function public.team_over_goals_first_half_streak_meta(
  p_league_id integer,
  p_goals_threshold numeric,
  p_n integer,
  p_required integer,
...
as $$
  ...
  select
    ...
    (select count(*) from agg where games = p_n)::integer as complete_windows_count,
    (select count(*) from agg where over_games >= p_required)::integer as qualified_teams_count;
$$;
