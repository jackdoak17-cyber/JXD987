-- Calculate team rankings for attacking and defensive stats
-- Used for opponent filtering in betting strategies
...
as $$
  ...
  team_aggregates as (
    ...
    from team_stats ts
    group by ts.team_id
    having count(distinct ts.fixture_id) >= (p_window * 0.5)::int -- At least 50% of window
  ),
  ranked_teams as (
    select
      ta.team_id,
...
-- Example usage:
-- Get top 5 defensive teams (least shots conceded) in Premier League:
-- select * from get_team_stat_rankings(8, 45, 10, null, 'defensive') limit 5;

-- Get top 5 attacking teams (most shots) in Premier League:
-- select * from get_team_stat_rankings(8, 45, 10, null, 'attacking') limit 5;
