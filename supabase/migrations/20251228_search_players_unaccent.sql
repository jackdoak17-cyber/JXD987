...
as $$
  ...
  normalized as (
    select
      unaccent(lower((select q from input))) as q_norm
  ),
  tokens as (
    select
      array_remove(regexp_split_to_array((select q_norm from normalized), '\s+'), '') as tokens
  ),
  latest_app as (
    select
      fp.player_id,
  ...
  matched as (
    select
      ...
      l.last_fixture_at,
      unaccent(lower(p.short_name)) = (select q_norm from normalized) as exact_short,
      unaccent(lower(p.common_name)) = (select q_norm from normalized) as exact_common,
      unaccent(lower(p.name)) = (select q_norm from normalized) as exact_name,
      case
        when coalesce(array_length(tokens.tokens, 1), 0) = 0 then false
        else (
          select bool_and(unaccent(lower(p.name)) like '%' || tok || '%')
      ...
      case
        when p.common_name is null or coalesce(array_length(tokens.tokens, 1), 0) = 0 then false
          ...
          select bool_and(unaccent(lower(p.common_name)) like '%' || tok || '%')
          from unnest(tokens.tokens) tok
        )
    ...
    where (select q_norm from normalized) <> ''
  )
  select
    player_id::integer,
    player_name::text,
