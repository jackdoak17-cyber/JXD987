...
as $$
  ...
  ranked as (
    select
      cards_per_team_fixture.*,
      row_number() over (
        partition by cards_per_team_fixture.team_id
        order by cards_per_team_fixture.starting_at desc, cards_per_team_fixture.fixture_id desc
      ) as rn
