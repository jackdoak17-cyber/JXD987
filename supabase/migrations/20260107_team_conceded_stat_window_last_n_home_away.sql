...
as $$
  ...
  ranked as (
    ...
    from appearances a
    where (
      (coalesce(p_home_only, false) = false and coalesce(p_away_only, false) = false)
      or (p_home_only is true and a.location = 'home')
      or (p_away_only is true and a.location = 'away')
    )
  ),
  windowed as (
    select *
